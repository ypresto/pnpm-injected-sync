...
import { syncInjectedDeps, type SyncInjectedDepsOptions } from '@pnpm/workspace.injected-deps-syncer'
...
interface SharedWatcherState extends WatcherState {
  clients: Set<number>
  lockFile: string
...
async function getInjectedDepsDirs(workspaceDir: string): Promise<string[]> {
  ...
  return Object.keys(modules.injectedDeps).map(relPath =>
    path.resolve(workspaceDir, relPath)
  )
}

async function syncDeps(pkgDir: string, workspaceDir: string): Promise<void> {
  if (isSyncDisabled()) {
    return
  }
...
async function acquireLock(lockFile: string, pid: number): Promise<boolean> {
  try {
...
async function tryPromoteToWatcher(lockFile: string, pid: number): Promise<boolean> {
  try {
...
async function registerClient(lockFile: string, pid: number): Promise<void> {
...
async function unregisterClient(lockFile: string, pid: number): Promise<number[]> {
  const clientFile = `${lockFile}.clients`
  let clients: number[] = []
...
async function cleanupStaleClients(lockFile: string): Promise<number[]> {
  const clientFile = `${lockFile}.clients`
  let clients: number[] = []
...
async function startWatcherMode(workspaceDir: string, lockFile: string, pid: number, getChildExited: () => boolean): Promise<SharedWatcherState> {
  ...
  const state: SharedWatcherState = {
    watchers: new Map(),
    debounceTimers: new Map(),
    clients: new Set([pid]),
    lockFile
...
async function runCommand(args: string[]): Promise<void> {
  ...
  let isWatcher = false
  let state: SharedWatcherState | null = null
  let promotionCheckInterval: NodeJS.Timeout | null = null
